...
(function() {
  ...
  function debugLog(message, data) {
    if (config.debug) {
      console.log('[Chat Widget Debug]', message, data || '');
    }
  }
  ...
  function saveConversation(messages) {
    try {
      ...
      console.error('[Chat Widget Error] No se pudo guardar la conversación:', e);
    }
  }
  ...
  function loadConversation() {
    try {
      ...
      console.error('[Chat Widget Error] No se pudo cargar la conversación:', e);
      return [];
    }
  }
  ...
  function sendMessage(text) {
    ...
    debugLog('Enviando mensaje:', { chatId: getChatId(), message: txt, route: config.webhook.route });
    ...
    .catch(err => {
      console.error('[Chat Widget Error] Fallo al enviar/recibir mensaje:', err);
      if (typing.parentNode) chatBody.removeChild(typing);
  ...
  function openWidget() {
    ...
    if (title) title.textContent = config.branding.title.split(' ')[0];
  ...
  function closeWidget() {
    ...
    if (title) title.textContent = config.branding.title;
  }

  // Inicialización (MEJORADA)
  function init() {
    try {
      debugLog('Iniciando widget con configuración:', config);
      injectStyles();
      buildWidget();
      ...
      debugLog('Widget inicializado correctamente.');
